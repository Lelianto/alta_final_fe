...
sudo docker build -f "Dockerfile" -t lelianto257/kutubuku:fe-$TRAVIS_BUILD_NUMBER  --label "Maintaner  Lelianto<lian@alterra.id>" .
# push apps image to docker hub
sudo docker push lelianto257/kutubuku:fe-$TRAVIS_BUILD_NUMBER
...
kubectl -n alta12 --kubeconfig kubeconfig set image deployment/${APP_K8S_DEPLOYMENT_NAME} ${APP_K8S_CONTAINER_NAME}=lelianto257/kutubuku:fe-${TRAVIS_BUILD_NUMBER}
